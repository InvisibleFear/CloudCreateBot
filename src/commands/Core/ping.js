...
export default {
    data: new SlashCommandBuilder()
        .setName("ping")
        .setDescription("Перевіряє затримку бота та швидкість API"),

    async prefixExecute(interaction) {
        try {
            const startTime = Date.now();
            const pingingMessage = await interaction.reply({ content: 'Визначення затримки...' });
            ...
            const embed = createEmbed({ title: 'Понг!', description: null }).addFields(
                { name: 'Затримка бота', value: `${latency}мс`, inline: true },
                { name: 'Затримка API', value: `${apiLatency}мс`, inline: true },
            );
            ...
            if (!interaction.replied && !interaction._replyMessage) {
                await interaction.channel.send({
                    embeds: [createEmbed({ title: 'Системна помилка', description: 'Не вдалося визначити затримку в цей час.', color: 'error' })],
                }).catch(() => {});
            }
        }
    },
