...
import { CLoudCreateError, ErrorTypes } from '../utils/errorHandler.js';
...
export class ModerationService {
  ...
  static assertModerationHierarchy(moderator, target, action) {
    ...
    if (!botCheck.valid) {
      throw new CLoudCreateError(botCheck.error, ErrorTypes.PERMISSION, botCheck.error);
    }

    const modCheck = this.validateHierarchy(moderator, target, action);
    if (!modCheck.valid) {
      throw new CLoudCreateError(modCheck.error, ErrorTypes.PERMISSION, modCheck.error);
    }
  }
